...
// Types mirrored from ../../types.ts to avoid pathing issues in build.
...
export const handler = async (event: { httpMethod: string; body: string | null }) => {
    ...
    try {
        const { text, numMcq, numTf } = JSON.parse(event.body || '{}') as RequestBody;

        if (!text || typeof numMcq !== 'number' || typeof numTf !== 'number') {
            return { 
                statusCode: 400, 
                body: JSON.stringify({ error: 'Missing required parameters: text, numMcq, and numTf must be provided.' }) 
            };
        }
          ...
          properties: {
            multiple_choice: {
              ...
              items: {
                ...
                properties: {
                  ...
                  answer: { type: Type.STRING, description: "The letter of the correct option (e.g., 'A', 'B', 'C', 'D')." },
                },
            ...
            true_false: {
              ...
              items: {
                type: Type.OBJECT,
                properties: {
                  question: { type: Type.STRING, description: "The statement for the true/false question." },
                  answer: { type: Type.STRING, description: "The correct answer, either 'True' or 'False'." },
                },
...
5.  Format the final output strictly as a JSON object according to the schema. Do not include any other text, explanations, or apologies.
    -   If you absolutely cannot extract any meaningful educational content to form questions, you must return a valid JSON object with empty arrays for 'multiple_choice' and 'true_false'.`;
        ...
        // This makes sure user-facing errors from the AI are passed through
        const errorMessage = error.message.includes("Could not generate meaningful questions")
          ? error.message
          : "Failed to generate quiz. The provided text might be too short, the content could not be processed, or there might be an issue with the API key.";
