...
// The Netlify handler for the serverless function.
export const handler = async (event: { httpMethod: string; body: string | null }) => {
    ...
    try {
        ...
        if (!text || typeof text !== 'string' || text.trim().length === 0) {
            return {
                statusCode: 400,
                body: JSON.stringify({ error: 'Input text is required and cannot be empty.' }),
                headers: { 'Content-Type': 'application/json' },
            };
        }
        
        // Initialize the Gemini client. The API_KEY is securely accessed from Netlify's environment variables.
...
`;
        ...
        if (quizData.multiple_choice.length === 0 && quizData.true_false.length === 0) {
            return {
                ...
                body: JSON.stringify({ error: "Could not generate meaningful questions from the provided text. Please try using a different source material with more descriptive content." }),
                headers: { 'Content-Type': 'application/json' },
            };
        }
        ...
        return {
            statusCode: 500,
            body: JSON.stringify({ error: "Failed to generate quiz. The provided text might be too short, the content could not be processed, or there might be an issue with the API key." }),
            headers: { 'Content-Type': 'application/json' },
        };
    }
};
